...
class TextNormalizationTools:
    ...
    def __init__(self) -> None:
        ...
        Sets up spaCy NER if available and initializes enhanced patterns.
        """
        # Try to load spaCy
        if SPACY_AVAILABLE:
            try:
                ...
                print("Warning: spaCy model 'en_core_web_sm' not available. NER tool will be disabled.")
        else:
            self.ner_available = False
        ...
        self.publishers_to_remove = {
            ...
            'BMG', 'BMG RIGHTS', 'KOBALT', 'CONCORD', 'IMAGEM', 'DOWNTOWN',
            'Deutsche Grammophon', 'Decca', 'Philips', 'Polydor', 'Capitol', 'Atlantic',
            'Island', 'Epic', 'Columbia', 'RCA', 'Parlophone', 'Virgin',
            ...
            'ADMINISTERED BY', 'ADMIN BY', 'OBO', 'C/O', 'DBA', 'AKA', 'TRADIONAL','MODERN'
            ...
            'MESAM', 'LIDO MELODIES', 'JUJUBA MUSIC', 'WHITE MANTIS PUBLISHING',
        ...
        self.terms_to_remove = [
            'traditional', 'modern', 'classical', 'contemporary',
            'pd', 'public domain', 'arrangement', 'arr', 'adapted', 'version',
            'remix', 'edit', 'mix', 'feat', 'featuring', 'vs', 'versus'
        ]
        ...
        self.publisher_keywords = [
            'publishing', 'publishers', 'publisher', 'music', 'records', 'record',
            'entertainment', 'limited', 'ltd', 'llc', 'inc', 'corp', 'corporation',
            'company', 'copyright', 'control', 'rights', 'management', 'sony',
            'universal', 'warner', 'bmg', 'emi', 'atlantic', 'columbia', 'editions',
            'edition', 'songs', 'musique', 'musikverlag', 'gmbh', 'productions',
            'international', 'group', 'label', 'imprint', 'catalog', 'catalogue'
        ]
        ...
        # Prefix pattern
        self.prefix_pattern = re.compile(r'^(CA|PA|PG|PP|SE|PE|MR|DR|MS|PD)\s+', re.IGNORECASE)

        # Name inversion pattern
        self.name_inversion_pattern = re.compile(r'^([A-Z][a-zA-Z\-\']+),\s*([A-Z][a-zA-Z\s\-\']+)$')
    ...
    def analyze_text_structure(self, text: str) -> str:
        ...
        try:
            ...
            has_compound_names = any(
                len(seg.strip().split()) > 1 and
                seg.strip().isupper() and
                all(len(word) >= 3 for word in seg.strip().split())
                for seg in segments
            )
    ...
    def remove_publishers(self, text: str) -> str:
        ...
        # Step 3: Enhanced publisher removal with better pattern matching
        ...
        for segment in segments:
            ...
            # Check if this segment is a publisher
            is_publisher = False
            segment_upper = segment.upper()

            # Direct match with publisher list (case insensitive)
            if segment_upper in self.publishers_to_remove:
                is_publisher = True
            else:
                # Check for partial matches BUT be more careful with short terms
                for publisher in self.publishers_to_remove:
                    # For short publisher terms (<=3 chars), require exact word match
                    if len(publisher) <= 3:
                        # Use word boundary matching for short terms
                        pattern = re.compile(r'\b' + re.escape(publisher) + r'\b', re.IGNORECASE)
                        if pattern.search(segment):
                            is_publisher = True
                            break
                    else:
                        # For longer terms, allow substring matching
                        if publisher in segment_upper:
                            is_publisher = True
                            break

            # Also check for compound publisher terms
            if not is_publisher:
                compound_pattern = re.compile(r'\b\w+(?:\s+\w+)*\s+(Records|Music|Publishing|Entertainment|Productions)\b', re.IGNORECASE)
                if compound_pattern.search(segment):
                    is_publisher = True

            # Keep non-publisher segments
            if not is_publisher:
    ...
    def fix_name_inversions(self, text: str) -> str:
        ...
        for segment in segments:
            ...
            match1 = re.match(r'^([A-Z][a-zA-Z\'-]+),\s*([A-Z][a-zA-Z\s\'-]+)$', segment)
            ...
            if match1:
                ...
                if (len(last_part.split()) <= 3 and len(first_part.split()) <= 3 and
                    not any(term in segment.lower() for term in ['inc', 'llc', 'ltd', 'corp'])):
                    # Keep as separate segments: LastName/FirstName
    ...
    def remove_prefixes(self, text: str) -> str:
        ...
        enhanced_prefix_pattern = re.compile(r'^(CA|PA|PG|PP|SE|PE|MR|DR|MS|C|P|PD)\s+', re.IGNORECASE)
    ...
    def split_compound_names(self, text: str) -> str:
        ...
        for segment in segments:
            ...
            if (len(words) > 1 and
                all(word.isupper() for word in words) and
                all(len(word) >= 3 for word in words) and
                not any(c.islower() for c in segment) and
                '-' not in segment):
    ...
    def validate_names(self, text: str) -> str:
        ...
        for segment in segments:
            ...
            is_valid = (
                segment and
                any(c.isalpha() for c in segment)  # Just needs to contain letters
            )
