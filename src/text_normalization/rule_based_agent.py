...
This module provides a hybrid approach to text normalization, combining
rule-based cleaning for simple cases with LLM processing for complex scenarios.
It focuses on realistic confidence assessment and honest performance evaluation.
"""
...
class HybridNormalizationAgent:
    ...
    def __init__(self) -> None:
        ...
        Sets up rule-based cleaner, NER model, and prepares LLM for
        complex cases that require additional processing.
        """
    ...
    def normalize_text(self, text: str) -> NormalizationResult:
        ...
        Uses rule-based cleaning first, then applies NER validation and
        LLM processing for complex cases. Provides honest confidence assessment
        based on evidence rather than artificial inflation.

        Args:
            text: Raw text to normalize (writer/composer names)

        Returns:
            NormalizationResult with normalized text, confidence, and reasoning
        """
    ...
    def _calculate_realistic_confidence(
        ...
        normalized: str,
        tools_used: List[str],
        ner_persons: List[str],
    ) -> float:
        """Calculate realistic confidence based on evidence, not artificial inflation.

        Evaluates confidence using multiple signals: text transformation quality,
        NER validation, tool effectiveness, and result plausibility.

        Args:
            original: Original input text
            normalized: Processed/normalized text
            tools_used: List of tools that were applied
            ner_persons: Person names identified by NER
        ...
        if ner_persons and "ner_validation" in tools_used:
            ner_coverage = sum(
                1 for person in ner_persons if person.lower() in normalized.lower()
            )
            if ner_coverage > 0:
        ...
        if (
            "various artists" in normalized.lower()
            and "various artists" in original.lower()
        ):
        ...
        if any(
            ...
            for keyword in ["control", "publishing", "music inc", "ltd"]
        ):
    ...
    def _needs_llm_processing(self, text: str, rule_result: str) -> bool:
        ...
        Only triggers LLM for genuinely complex cases where rule-based
        processing is likely to fail or has clearly failed.

        Args:
            text: Original input text
            rule_result: Result from rule-based processing

        Returns:
            True if LLM processing is recommended
        """
        ...
        if (
            "various artists" in text.lower()
            and "various artists" not in rule_result.lower()
        ):
            return True

        # 3. Name ordering issues (LastName, FirstName)
        if any("," in part and len(part.split(",")) == 2 for part in text.split("/")):
            return True
    ...
    def _process_with_llm(self, text: str) -> Optional[str]:
        ...
        Uses carefully crafted prompts with examples to guide the LLM
        in normalizing complex text cases that rules cannot handle.

        Args:
            text: Input text to process with LLM

        Returns:
            Normalized text from LLM, or None if processing fails
        """
        if not self.llm:
            return None
