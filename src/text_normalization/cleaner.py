...
class RuleBasedCleaner:
    ...
    def _compile_patterns(self):
        ...
        self.publisher_pattern = re.compile(rf"\b({publisher_terms})\b", re.IGNORECASE)
    ...
    def clean_text_with_confidence(self, raw_text: str) -> CleaningResult:
        ...
        confidence = self._calculate_rule_based_confidence(
            raw_text, cleaned_text, applied_rules
        )
        needs_review, review_reasons = self._assess_review_need(
            raw_text, cleaned_text, confidence
        )
    ...
    def _calculate_rule_based_confidence(
        self, original: str, cleaned: str, applied_rules: List[str]
    ) -> float:
    ...
    def _assess_review_need(
        self, original: str, cleaned: str, confidence: float
    ) -> Tuple[bool, List[str]]:
        ...
        complexity_score = sum(
            1 for indicator in complexity_indicators if indicator in original_lower
        )
